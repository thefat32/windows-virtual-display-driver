...
VOID Log(LPWSTR text) {
    ...
    StringCchCat(filePath, MAX_PATH, TEXT("\\pepe.log"));
    CHandle hFile(CreateFile(filePath, FILE_APPEND_DATA, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL));
    SetFilePointer(hFile, 0l, nullptr, FILE_END);
    DWORD dwBytesToWrite = ((DWORD)wcslen(text) * sizeof(*text));
